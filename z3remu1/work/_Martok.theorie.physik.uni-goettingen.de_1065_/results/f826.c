...
#include"num_out.h"
#include"num_in.h"
extern FNN F826_p19p19_p19a19a19_u;
...
REAL F826_p19p19_p19a19a19_u(double GG,REAL*DP,REAL*Q0,COMPLEX*Q1,REAL*Q2,REAL*
 cb_coeff)
{
...
if(!DP){C826(C); return 0;} 
  REAL N_p1p2_=1/DP[0];
N=+C[0];
D=+9;
R=+DP[2]*(2*(DP[3]-DP[1]-DP[2])+C[1]+DP[0]+DP[4])+DP[4]*(DP[3]-C[1]-DP[0]-
 DP[1]+DP[4])+DP[0]*(DP[1]-DP[3])+C[1]*(DP[1]-DP[3]);
R*=(N/D);
