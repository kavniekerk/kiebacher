...
static double vsigma32integrandT(double *x, double w) //32add
{
   double pcmIn,sqrtS,M45;
   double M45_min=pmass[i4]+pmass[i5];
   double M45_max=sqrtSmax-pmass[i3];
   int err;
   double r, x1,x2,y,z, bess,Rm;
   double GG;
   REAL pvect[20];
   ...
   x1=M1/T_; x2=M2/T_; y=sqrtS/T_;
   //if(y-x1-x2>50) return 0;//activate for faster calculation loosing accuracy

   r=kinematic_32(pcmIn,i3, M45, 2*x[2]-1 ,2*x[3]-1,M_PI*x[4],pmass,pvect)*8*M_PI*(M45_max-M45_min); //  /pcmIn
   if(r==0) {return 0;}
   GG=sqrt(4*M_PI*alphaQCD(sqrtS));
   r*= CI->sqme(1,GG, pvect,NULL,&err);
   //if (!err) printf("fck\n");
...
double vSigmaCC32 (double T,numout* cc, int mode)//32add
{
  int i,err,n,n0,m,w;
  char*s, *pname[6];
  int pdg[6];
  ...
  { 
     ...
     {  
        ...
        if(T==0) a=vegas_chain(3, vsigma23integrand0 ,2000,1., 0.03,&dI,NULL);
            ...
            {
            //a=vegas_chain(5, vsigma32integrandT ,2000,1., 0.03125,&dI,NULL);
            int jj;
